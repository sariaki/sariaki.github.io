...
## Introduction
I spent the past ~7 months coming up with different creative ideas for a national research/science fair competition in Germany ("Jugend forscht"). After being inspired by Brit of secret.club fame, I ultimately ended up right back at home where I had begun years prior -- Reverse Engineering/Game hacking.
...
### Symbolic Execution
Imagine trying to prove that the above opaque predicate is actually opaque so that you can remove it as well as the path B which is never taken. You might:
...
The problem with this method lies in it's high rate of false positives between ~20% and ~40% [^1]. Allthough all opaque predidicates might be removed from a binary, valuable legitimate code will be removed as well.

This leaves us with option 3. For this, one would have to show that $\forall x \in \mathbb{R} : 2 | x \cdot (x+1)$ since: (1) an even number multiplied by an odd number is always even. (2) if $x$ is even, $x \cdot (x+1)$ must be even by (1). (3) if $x$ is odd, $x \cdot (x+1)$ must also be even since $x+1$ must be odd and (1) still applies $\square$.
...
## Motivation
<!-- Current SOTA opaque predicates aimed to defeat symbolic execution can in my opinion be divided into two main categories:
1. ...

2. ... -->
Current SOTA opaque predicates aimed to defeat symbolic execution currently either fail to resist symbolic execution based attacks or are simply to slow / easily identifyable. Considering that so many obfuscators implement opaque predicates to be paired with other obfuscation methods, this becomes relevant.
Below are some of the current opaque predicates used:
- A
- B
- C
...
### Construction
Using uniformly distributed (pseudo-)random variables allows for reverse engineers to easily understand which branch is practically taken. To make this more difficult, I've created an algorithm which generates a new random probability distribution for each POP. Here's the summary:

Let $f(x, y, z, ...)$ be the function we're trying to obfuscate.

1. First, we create a statically undefined symbolic variable for the solver. Traditionally, existing implementations would simple use a random argument of $f$ to achieve this. However, for functions only ever called with constant parameters, this becomes an issue: an attacker could simple execute all function predicates with every possible input that comes up in the binary to check if the same branch is always taken.
To combat this, I created a DFS-algorithm which walks the use-def chain to find find out if the parameter is actually statically indeterminate. This is the case if any assignment of our parameter $x$ depends on an external function/global or similar -- anything that can't be reduced to a small set of possible values.

<p align="center">
  <img src="/posts/img/use-def chain tree.png" width=300rem//>
  <p align="center">An example for a use-def chain tree. The red highlights show a path from our parameter to an undefined external value.</p>
</p>
...
3. By computing the inverse of our CDF $y=B_n^{-1}(x)$ [^2] with our symbolic variable $x$, our output $y$ is distributed the same as $B_n(x)$'s underlying probability density function (PDF). This is known as the inverse transform method [^3]
...
4. Since we control the CDF, we can chose what value-ranges of our new variable $y$ are probable/improbable and make probable/improbable statements based on that information.
...
This, of course, also works for discrete distributions.

## Benchmarks
All benchmarks were performed using clang 18.1.3 with `-O3` and Ubuntu 24.04.3 LTS with kernel version 5.16 as well as an Intel&#174; Core&#8482; i7-10700F CPU and 16Gb DDR4 RAM. 31 Programs from the LLVM Test Suite [^4] were tested 5 times. Since I get to chose the probability of failiure, I chose 0.000001%.
<figure>
...
During the presentations I've held before friends as well as others interested, I often get asked about whether the POP obfuscation pass can programs fail in practice. Based on my tests, I can confidentely say that this doesn't happen. All tests built by competent compiler engineers smarter than me for checking if program semantics stay intact continue to pass.
...
As is to be expected, runtime costs and program size increase with more obfuscation applied. The median runtime performance of a binary with 100% of all functions obfuscated is ~15% slower than its unobfuscated counterpart. The median program size with all functions obfuscated rises by ~25%. For most use cases, this should be fine. A bigger problem might be that the distance between the upper and lower quartiles is fairly large. For instace: With 100% of functions obfuscated, the slowest 25% of programs were more than two times as slow as they were unobfuscated. The same goes for their size. This might seem dramatic at first glance but can be explained by some of the program's sizes being relatively small: Since our POPs don't change size relative to the programs, they'll obviously have a larger impact on smaller ones.
...
I won't go into detail how I measured stealth here, allthough I found it to be quite interesting. The important part here is that the main shortcoming POPs face is that they're statistically detectable. This can be traced back to the plethora of floating point instructions they use at once which might seem uncommon depending on the program. This is bad since once an attacker realises that a predicate is actually a POP, he can simply execute it once to find the right path and remove the POP. To combat this, users show as of right now pair the control flow obfuscation detailed with junk code.

## Conclusion
In summary, probabilistic opaque predicates provide a novel way to combat opaque predicates' main weakness, symbolic execution. In doing so, when configured/paired correctly, they hold practical costs, strength, resilience and stealth. I hope for others to develop new methods to attack POPs as well as to use this for inspiration to maybe not just create their own obfuscator but maybe even devlop their own obfuscation methods.
Although I spent tons of time on this which I could've more efficiently allocated towards studying for the German finals, I'm happy with what I created and hope for others to find it useful.

**Incase you feel like reaching out at this point: My Discord is @sariaki** (uid 671779079363624970)
